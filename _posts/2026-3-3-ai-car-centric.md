---
layout: post
title: AI-first Societies are like Car-centric Cities
published: true
category: main
---

{{ page.title }}
================

<p class="meta">3.3.2026 - SF Bay area</p>

Last year my husband and I moved from the SF Bay area to Austin, TX, in order to be closer to my parents in Houston. 

I grew up in Texas, but this was the first time I'd actually lived there longer than 2 months since I was 21. In the interim, I've lived in cities such as Chicago, Washington DC, and San Francisco, with frequent trips to London, NYC, and other big metropolises. When I stepped foot back in Texas, I actually felt a bit like a foreigner. There were good things, such as rediscovering the wonderful Tex-Mex (the first chile relleno plate I had in Austin was so nostalgic and amazing)(have to also mention the incomparable Texas Roadhouse, we basically inhale those dinner rolls), or the cicadas calling from the verdant oaks in summer. But the flip side, in my reverse-culture shock, was the car-centric nature of Texas.

![Texas traffic](/images/mopac-traffic.png)

Now, Austin's not that bad. But I'd gotten so used to pedestrian cities that when someone stole my wallet around 2015 in DC, I didn't bother replacing my drivers license. I didn't drive for over ten years between the time I left Texas in 2011 and the end of the pandemic. I was used to walking to grocery stores, restaurants, shopping, gyms, meeting up with friends, offices, and Ubering/Waymo-ing/taking the train anywhere else. 

Suddenly, we had to get in a car and deal with intersections, highways, and other drivers in order to get anywhere. Don't even get me started on traffic jams. It was clear that these Texas cities, built with the rise of the personal vehicle and highways, were rather inefficient at moving people. And I think, for all the talk of the Texas boom, that this holds back their economies compared with pedestrian cities.

### The AI Highway

I mention this because I see a very similar parallel with the rise of AI. Everywhere (at least in the tech world), people are talking of building AI-first companies, AI-first cities, AI-first economies. Well, we've already had this experiment. With the rise of one of the most disruptive technologies of its time, the personal vehicle, we also started building car-first cities, car-first economies. We paved over neighborhoods and built soaring highways between them. We cut down trees and expanded our highways from one lane, to two, to five and six. We remodeled our cities so that people lived out in the suburbs and commuted for long times to their offices, and slowly never saw their neighbors.

At the end of it all, the cities and economies that thrived the most were the ones that stuck to a more natural flow -- the pedestrian cities built before the rise of the car. New York City, Tokyo, London, etc. -- they kept their density, their 15-minute essentials, their parks and shops. What was 'traditional' and 'old school' served them in the long run. Not to say they don't have cars -- they certainly do. But cars did and do not take precedence over people in those cities.

I believe AI-first companies and economies are similar. I have no doubt some people, some companies and cities may reorient themselves and/or build anew with this AI orientation. I believe they will generate returns and economic activity. 

But I think they will trail the companies, economies, and societies that preserve the human-centric way of doing things, with AI only as accoutrement. For they will find, in an AI-first world, that people will find it hard to connect with other people, that there will be a lot of AI-generated noise, that races occur to both the top and bottom, that things actually don't get more efficient.

The ones that emphasize in-person meetings, mentorship, human-generated writing and communication, rather than offloading it all to AI, will, I believe, succeed in the long run.

Oh, and as for Texas? We didn't last a year ... we moved back to the Bay area.
